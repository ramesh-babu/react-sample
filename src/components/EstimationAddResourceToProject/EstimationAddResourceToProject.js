...
import { includes, uniq, map } from 'lodash';
import MuiThemeProvider from 'material-ui/styles/MuiThemeProvider';
import ReactDOM from 'react-dom';
import DatePicker from 'material-ui/DatePicker';
import { Row, Col } from 'react-bootstrap';

export default class EstimationAddResourceToProject extends Component {
  static propTypes = {
    estimationAppendNewResource: PropTypes.func,
    estimationRemoveCurrentResource: PropTypes.func,
    estimationHandleResourceData: PropTypes.func,
  ...
  handleFieldChange = (name, value) => {
    ...
    temp[name] = value;
    this.props.estimationHandleResourceData(this.props.itemIndex, temp);
  };
  handleField = (name, value) => {
    const empId = uniq(map(this.props.resources, 'emp_id'));
        ...
        <Col md={1} lg={1} sm={1}>
          <div className="form-group">
            <input
              ...
              onClick={ (this.props.itemIndex + 1) === this.props.resources.length ? this.props.estimationAppendNewResource : () => { this.props.estimationRemoveCurrentResource(this.props.itemIndex); } }
            />
            <label></label>
          </div>
        </Col>
